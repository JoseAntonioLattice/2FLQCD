...
\section{SU($N$) gauge theory}
    ...
    \begin{equation}
        U = e^{iA} \in \SU{N}
    \end{equation}
where $A\in\su{N}$ is traceless and Hermitian, an element of the Lie algebra. There are $N^2-1$ generators $T^a$ of the algebra, and they satisfy
...
Link variables are oriented, so they naturally live in the links between neighboring points on the lattice. Thus, the variable $U_{\mu}(x)$ is defined in the link between points $x$ and $x + \hat \mu$ in the positive $\mu$ direction, see Fig.\ \ref{fig:link}. We define the variable between points $x$ and $x + \hat \mu$ pointing in the negative direction via
\begin{equation}
...
\subsection{Leapfrog}
    \begin{enumerate}
        ...
        \item Initial step
            \begin{equation}
                P_{1/2} = P_0 - \left.\frac{\epsilon}{2}F[U,\phi]\right|_{U_0}.
            \end{equation}
        \item Intermediate steps
        For $k = 1, \dots, n - 1 $:
            \begin{equation}
                U_{k} =  \exp\left( i\epsilon P_{k-\frac{1}{2}}\right)U_{k-1} , \ \ \ P_{k+\frac{1}{2}} = P_{k-\frac{1}{2}} - \left.\epsilon F[U,\phi]\right|_{U_k}
            \end{equation}
        \item Final step
            \begin{equation}
                U_{n} =  \exp\left( i\epsilon P_{n-\frac{1}{2}}\right)U_{n-1} , \ \ \ P_{n} = P_{n-\frac{1}{2}} - \left.\frac{\epsilon}{2} F[U,\phi]\right|_{U_n}
            \end{equation}
...
\begin{equation}
    F[U,\phi] = \sum_{a = 1}^8 T^a \nabla^{(a)} \left(S_G[U] + \phi^{\dagger}(DD^{\dagger})^{-1}\phi \right) \in \su{3}
\end{equation}
